...
func newTasksCommand(s *state) *cobra.Command {
	...
	all.Flags().StringVar(&status, "status", "", "Task status: active or all (for completed tasks use: wsectl tasks search --status done)")
	...
	list.Flags().StringVar(&listStatus, "status", "", "Task status: active or all (for completed tasks use: wsectl tasks search --status done)")
